...
# Hapus semua ENV RCLONE_* supaya tidak override config manual
...
# Jalankan initial sync di background (tidak blocking)
echo "Starting initial sync from B2 in background..."
...
# Jalankan Paperless-ngx (entrypoint bawaan)
echo "Starting Paperless-ngx server (entrypoint)..."
exec /entrypoint.sh
